...
class AdminAddSongPage extends StatelessWidget {
  const AdminAddSongPage({super.key});
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: () async {
                ...
                bool isAdded = await AdminLogic().addSongInfo(
                  songName,
                  mainArtist,
...
class _AddSongFileFormState extends State<AddSongFileForm> {
  ...
  Future<void> _uploadFileWithProgress(File file) async {
    //AdminLogic songService = AdminLogic();
    ...
    try {
      ...
      // Call the method to handle adding songs from file with progress
      List<int> result =
          await AdminLogic().addSongFileWithProgress(file, onProgress);
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: () async {
                ...
                bool transferResult = await AdminLogic()
...
class _AddSongSpotifyFormState extends State<AddSongSpotifyForm> {
  ...
  //final AdminLogic songService = AdminLogic(); // Instance of your SongService
  ...
  void searchSpotify(String query) async {
    ...
    try {
      var results = await AdminLogic().fetchSpotify(query);
      setState(() {
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            TextButton(
              ...
              onPressed: () async {
                Navigator.of(dialogContext).pop();
                bool success = await AdminLogic().saveSpotifySongToDb(song);
